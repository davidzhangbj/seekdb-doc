...
For more information, see [seekdb Agent Skill](https://www.oceanbase.ai/docs/agent-skill).

## Prerequisites

* **Node.js**: Version 18 or later.

* **Network access**: You can access GitHub (required to install the
plugin and fetch the latest documentation).

* **Claude Code** must be installed before you install the seekdb plugin. If it is not installed, follow the [Claude Code documentation](https://code.claude.com/docs/en/overview#native-install-recommended). The recommended method is **Native Install**. On macOS, Linux, or WSL you can run:

    ```shell
    curl -fsSL https://claude.ai/install.sh | bash
    ```

    For other platforms (e.g. Windows) and options like Homebrew or WinGet, see the documentation above.
...
Setting `skipWebFetchPreflight` to `true` skips the preflight check for network requests. The plugin prefers local documentation. When the relevant documentation is not available locally, it falls back to fetching from GitHub, which requires network access.

:::

## Step 2: Install the seekdb plugin

You can install the seekdb plugin in either of two ways:

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<Tabs>
  <TabItem value="Plugin marketplace" label="Plugin marketplace" default>
  ...
  3. Restart Claude Code.
     ...
     After installation, exit and restart Claude Code so the plugin loads correctly.

  </TabItem>
  <TabItem value="pip install" label="pip install">

  1. Go to the install directory.

     Skills are installed under the directory where you run the install command. Change to your project root, then run the steps below.
  ...
  4. Select the tool.
     ...
     In the installer, use **↑/↓** to choose **Claude Code**, then press **Enter** to confirm.

  5. Confirm install location.

     The installer shows the project root (skills will be installed under `.claude/skills` in that directory). When prompted `Install skills to this directory?`, enter **Y** to continue or **n** to exit.

  6. Select skills to install.

     **seekdb** (selected by default): documentation knowledge base covering official seekdb docs so the AI can look up answers from the docs.

  7. Start Claude Code.

     After installation, start Claude Code from the project root where skills were installed and use it as usual.
