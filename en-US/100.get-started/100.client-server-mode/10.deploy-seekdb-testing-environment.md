...
# Get started with server mode of seekdb by using SQL

seekdb supports SDK-based and SQL-based usage. This topic describes how to get started with server mode of seekdb by using SQL.

:::info
For more information about how to use seekdb by using an SDK, see [Get started with embedded seekdb by using the Python SDK](../50.embedded-mode/25.using-seekdb-in-python-sdk.md).
:::

In this example, we perform the following operations:

1. Deploy server mode of seekdb.
2. Connect to seekdb.
3. Create a database.
...
## Step 1: Deploy seekdb in server mode
...
* MySQL client is installed in your environment.

* The user you are using has sudo privileges.

* The following conditions are met for deployment using yum install:
...
* The following conditions are met for deployment using Docker:
...
Create a database named `my_test`. You can also use the default test database of seekdb for experience.
...
2. Switch to the custom database.

    ```sql
    use my_test;
    ```

## Step 4: Create a sample table and insert data
...
* [Experience the Vibe Coding paradigm with Cursor Agent and OceanBase MCP](80.experience-vibe-coding-paradigm-with-cursor-agent-oceanbase-mcp.md)
