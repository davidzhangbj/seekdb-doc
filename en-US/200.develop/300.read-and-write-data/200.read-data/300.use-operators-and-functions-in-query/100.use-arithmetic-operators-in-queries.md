...
## slug: /use-arithmetic-operators-in-queries

# Use arithmetic operators in queries

This topic provides examples of how to use arithmetic operators in queries.

## Arithmetic operators

Common arithmetic operators in queries include + (addition), - (subtraction), * (multiplication), / (division), - (unary minus), and MOD (modulo). These operators can be applied to columns of numeric data types.
...
Query the number and price of items that meet the criteria. The total payment for each category is the product of the quantity and price. You can add a column `t2.ol_quantity * t3.i_price` to the `select_list` and rename it to `item_sum_price`.

```sql
SELECT t1.c_first, t1.c_last, t1.c_credit, t2.o_ol_cnt * t3.i_price item_sum_price, t2.o_entry_d, t3.i_name, t3.i_price 
      FROM cust t1
