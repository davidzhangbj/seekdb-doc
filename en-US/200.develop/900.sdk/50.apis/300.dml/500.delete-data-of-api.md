...
|`where`|dict|Optional|The metadata filter. You can specify a single metadata filter or an array of metadata filters. For more information, see [Metadata filter](../100.data-api.md#metadata-filter).|`{"category": {"$eq": "AI"}}`|
|`where_document`|dict|Optional|The document filter. You can specify a single document filter or an array of document filters. For more information, see [Document filter](../100.data-api.md#document-filter).|`{"$contains": "obsolete"}`|
