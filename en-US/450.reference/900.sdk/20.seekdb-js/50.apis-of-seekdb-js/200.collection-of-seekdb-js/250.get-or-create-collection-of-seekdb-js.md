...
* You have installed the server mode of seekdb. For more information about how to install the server mode of seekdb, see [Deploy by using yum install](../../../../../400.guides/400.deploy/700.server-mode/100.deploy-by-systemd.md).

* You have connected to the database. For more information about how to connect to the database, see [SeekdbClient](../50.seekdbclient-of-seekdb-js.md).
...
* We recommend that you use a meaningful name for a table instead of a generic name such as t1 or table1. For more information about table naming conventions, see [Table Naming Conventions](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000003977289).
...
|`configuration`| Configuration \| HNSWConfiguration |No|The configuration of the collection. `Configuration` contains two attributes: `HNSWConfiguration` and `FulltextAnalyzerConfig`.|`{hnsw: {dimension: 384, distance: 'cosine'}, fulltextConfig: {analyzer: 'ik', ik_mode: 'smart'}}`|
