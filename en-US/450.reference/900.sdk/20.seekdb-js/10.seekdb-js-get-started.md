---
slug: /seekdb-js-get-started
---

# Quick start

## seekdb-js

seekdb-js is a node client provided by OceanBase Database. It allows you to connect to a remote server (in server mode) or OceanBase Database through a set of APIs.

:::tip
OceanBase Database is a fully self-developed enterprise-level native distributed database independently developed by OceanBase. It achieves financial-grade high availability on general-purpose hardware. It sets a new standard for automatic lossless disaster recovery across three regions with five IDCs. It also sets a new benchmark in the TPC-C standard test with a single cluster size exceeding 1,500 nodes. OceanBase Database is cloud-native, highly consistent, and highly compatible with Oracle and MySQL.
:::

## Install seekdb-js

### Prerequisites

Make sure that your environment meets the following requirements:

* Node.js 20 or later

* The server mode of seekdb has been deployed and started. For more information about how to deploy the server mode of seekdb, see [Deploy the server mode of seekdb by using yum install](../../../400.guides/400.deploy/700.server-mode/100.deploy-by-systemd.md).

### Procedure

```shell
npm install seekdb
```

## What to do next

* After you install seekdb-js, you can connect to seekdb to perform operations. For more information about the APIs supported by seekdb-js, see [API Reference](50.apis-of-seekdb-js/10.api-overview-of-seekdb-js.md).

* You can also refer to our SDK samples to quickly get started with seekdb-js.

  * [Simple sample](30.sdk-samples-of-seekdb-js/10.seekdb-js-simple-sample.md)

  * [Complete sample](30.sdk-samples-of-seekdb-js/50.seekdb-js-complete-sample.md)

  * [Hybrid search sample](30.sdk-samples-of-seekdb-js/100.seekdb-js-hybrid-search-sample.md)