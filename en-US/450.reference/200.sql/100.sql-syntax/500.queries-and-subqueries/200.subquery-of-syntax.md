...
The preceding query returns a value of the `VARCHAR` type with a length of `20`, `OceanBase`, which is in the default character set and collation of the tenant. The nullability of the value selected by a scalar subquery is not copied. Although the `c2` column has a `NOT NULL` constraint, if the result of the subquery is `NULL`, the result will still be `NULL`.
...
For more information about lateral derived tables, see [Lateral derived tables](../../../../../zh-CN\200.develop\300.read-and-write-data\200.read-data\200.multi-table-join-query\600.lateral-derived-tables.md).
