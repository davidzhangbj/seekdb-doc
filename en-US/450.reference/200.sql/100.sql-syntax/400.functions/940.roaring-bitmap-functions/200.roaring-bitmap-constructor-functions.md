...
The RoaringBitmap constructor functions are used to construct new RoaringBitmap data. The RoaringBitmap constructor functions supported by OceanBase Database include `rb_build()`, `rb_build_empty()`, `rb_build_varbinary()`, and `rb_from_string()`.

  :::info

  For more information about the considerations for visualizing the hexadecimal query results, see <a href="../../100.basic-elements/100.data-type/1000.roaring-bitmap-data-type/200.create-roaring-bitmap-columns.md">Create a RoaringBitmap column.
...
The `rb_build_varbinary()` function is used to construct a RoaringBitmap from a varbinary string. Varbinary is a proprietary format of OceanBase Database, which is a binary format that consists of version information, type information, and data. The syntax is as follows:
