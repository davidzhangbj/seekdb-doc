...
The `format` parameter is optional. Currently, only `roaring` is supported. After configuration, the output binary data types are only compatible with BITMAP_32 and BITMAP_64. If the `format` parameter is not configured, the default output format is the `varbinary` format of OceanBase Database, which includes seven subtypes: BITMAP_32, BITMAP_64, BITMAP_16, BITMAP_8, BITMAP_4, BITMAP_2, and BITMAP_1.
