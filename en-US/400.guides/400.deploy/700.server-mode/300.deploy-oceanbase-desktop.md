...
* Install using dmg installation package

  1. Install the dependencies

      macOS requires OrbStack to install OceanBase Desktop. OrbStack is a modern container and VM management tool designed for macOS. It provides efficient container runtime, such as Docker, and lightweight virtual machines. Its goal is to replace Docker Desktop and other cumbersome virtualization tools while significantly improving performance, saving system resources, and simplifying the developer experience.

      :::info
         OrbStack offers two methods to install its CLI: downloading the package and using the brew command. Here, we provide instructions for installing by downloading the package. For other methods, please refer to <a href="https://docs.orbstack.dev/quick-start">the official tutorial</a>.
      :::

     1. Download the installation package.

        You can visit [official website download link](https://orbstack.dev/download) to download the installation package. This link provides two packages, one for Apple Silicon chips and another for Intel chips. To determine the chip type of your computer, click the Apple icon in the upper-left corner of the screen and then select **About This Mac**.
     ...
     2. Installation
        ...
        Double-click the dmg package and drag the icon to the Applications folder to install it.

        :::tip
          For the first-time installation, make sure that you have completed the initial boot process of OrbStack, or you cannot import the image.
        :::

        ![Install](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.3.5/400.deploy/800.deploy-oceanbase-desktop/800.deploy-oceanbase-desktop-06.png)

     3. (Optional) Verify whether the installation was successful.

        In the Machines (Virtual Machines) section, select Install a new image from the web and choose an Ubuntu image. This process may take some time, so please be patient.
        ...
        After installation is complete, double-click the corresponding virtual machine. If you can enter the command-line interface, OrbStack has been installed successfully.
  ...
  2. Install OceanBase Desktop
      ...
      1. Download the installation package
         ...
         Go to [OceanBase Download Center](https://en.oceanbase.com/softwarecenter) and download the **OceanBase Desktop One-click Installation** package for Mac OS (Version 1.3).


      2. Install applications

         Click the installation package and drag the icon to the Applications folder to install it.

      3. Start OceanBase Desktop.

         After the installation is complete, a stopped seekdb is automatically deployed. You can click the **Start** button on the graphical interface to start seekdb.
