...
* Solution: Please use the `SET variable=expression, ...` or `SELECT expression(s) INTO variables(s)` syntax.
...
* Cause:
  ...
  * The input plugin type is supported (seekdb currently only supports the `mysql_native_password` type), but the plugin was not correctly loaded by the program.
...
* Solution: Ensure that the syntax of the `ALTER EVENT` statement is correct, including the attributes and values to be modified.
...
* Cause: The attempt to delete the event from the `mysql.event` table failed.
