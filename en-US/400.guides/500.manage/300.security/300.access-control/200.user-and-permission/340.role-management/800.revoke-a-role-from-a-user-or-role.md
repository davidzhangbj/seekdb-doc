...
* Revoke the `employee` and `developer` roles from the `test1` user.
  ...
  In this example, the `IF EXISTS` clause is used to check whether the roles to be revoked exist in the tenant. If you do not specify the `IF EXISTS` clause, an error is returned if either the `employee` or `developer` role does not exist. If you specify the `IF NOT EXISTS` clause, a warning is returned.
