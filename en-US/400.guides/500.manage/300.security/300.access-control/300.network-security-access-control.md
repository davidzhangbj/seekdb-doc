...
The whitelist is controlled by the system variable `ob_tcp_invited_nodes`, which is a global parameter. The default value is `127.0.0.1,::1`, indicating that only the IP address of the current server can connect to the tenant. This variable supports list values, which are separated by commas (,). For example, A,B,C,D. When a user logs in, seekdb will match the user's IP address with A, B, C, and D in sequence. If none of them match, the access is denied. If any of them match, the access is allowed.
