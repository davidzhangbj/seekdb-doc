...
* Log in to seekdb using a MySQL client and query the `ssl_cert_expired_time` field in the `oceanbase.GV$OB_SERVERS` view under the sys tenant to confirm if encryption is enabled.
...
:::tip
For RPC connections, the changes take effect only after the cluster is restarted, or if existing RPC connections are disconnected without restarting the cluster.
:::