...
1. Download the TPC-H tools from the [TPC-H Tools download page](https://www.tpc.org/tpc_documents_current_versions/download_programs/tools-download-request5.asp?bm_type=TPC-H&bm_vers=3.0.0&mode=CURRENT-ONLY).
...
3. Adjust query parallelism in the generated SQL files as needed.
...
You can write a load script based on the generated data files and SQL statements. The following example shows a commonly used approach.

1. Create a script named `load_data.sh`:

   ```bash
   #!/bin/bash
   host='$host_ip'   # Note: Use the IP of the seekdb host. We recommend placing the data files on that host as well.
   port='$host_port' # seekdb port.
   user='$user'      # Username.
   password='$password'   # Password.
   database='$db_name'    # Database name.
   data_path='$data_file' # Path to the .tbl files generated in Step 4.

   function load_data
   {
   remote_user="$user" # Username on the seekdb host where the data files are stored.
   table_name=${1}
   if [[ ${password} == "" ]]; then
   ...
   # You can replace the following statement with another loading method if needed.
   # Note: The data files must be stored on the seekdb host because the load is local.
...
2. Run a major compaction:

   ```sql
   ALTER SYSTEM MAJOR FREEZE;
   ```

3. Check whether the major compaction is complete:
...
1. Create a script named `tpch.sh`:
   ...
   ```bash
   #!/bin/bash
   host='$host_ip'   # Note: IP of the seekdb host.
   port='$host_port' # seekdb port.
   user='$user'      # Username.
   password='$password'   # Password.
   database='$db_name'    # Database name.
