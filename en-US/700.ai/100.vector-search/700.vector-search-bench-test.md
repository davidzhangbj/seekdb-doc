...
* Deploy SeekDB.
* Install Python 3.11 or later. The following example demonstrates how to install Python 3.11 using Conda:
  ```bash
...
* Connect to the database and optimize memory and query parameters for HNSW vector index searches:
  ...
  ```
  Here, `ob_vector_memory_limit_percentage = 30` is an example value. Adjust it based on the database memory and workload.
...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>We recommend that you deploy VectorDBBench and SeekDB on separate servers to avoid CPU resource contention and improve the reliability of test results.</p>
</main>
