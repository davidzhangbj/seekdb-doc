...
* For more information about vector indexes and examples, see [Create vector indexes](200.vector-index/200.vector-index.md).
* To perform large-scale performance tests, we recommend that you use the [VectorDBBenchmark tool](700.vector-search-bench-test.md) to generate a test dataset to better compare the performance differences between exact search and approximate search.