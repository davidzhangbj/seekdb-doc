# Integrate seekdb Vector with Jina AI

seekdb provides vector storage, vector indexing, and embedding vector retrieval capabilities. You can store the vectorized data in seekdb for subsequent retrieval.
...
Obtain the [Jina AI API key](https://jina.ai/api-dashboard/reader) and configure the environment variable with the seekdb connection information.
...
Jina AI provides various embedding models. You can choose the model that meets your needs.
...
Here is an example of defining a `generate_embeddings` helper function to call the Jina AI embedding API using the jina-embeddings-v3 model:
...
Create a table named `jinaai_oceanbase_demo_documents` with columns for storing text (`content`), embedding vectors (`content_vec`), and vector index information. Store the vector data in seekdb:
...
Generate an embedding vector for the query text using the Jina AI embedding API, and search for the most relevant documents based on the cosine distance between the query embedding vector and each embedding vector in the vector table:
