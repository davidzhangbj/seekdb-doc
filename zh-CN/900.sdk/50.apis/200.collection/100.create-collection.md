...
* 你已部署 seekdb 或者嵌入式 seekdb。

* 如果您使用的是客户端模式的 seekdb 或者 OceanBase 数据库，请确保连接的用户具备 `CREATE` 权限。查看当前用户权限的相关操作请参见 [查看用户权限]()。如果不具备该权限，请联系管理员为您授权，用户授权的相关操作请参见 [直接授予权限]()。
...
# 创建具有 embedding function 的 collection（传统方式）
collection = client.create_collection(
    name="my_collection",
)

# 使用默认 embedding function 创建 collection（自动计算维度）
ef = UserDefinedEmbeddingFunction() // define your own Embedding function, See section.6
config = HNSWConfiguration(dimension=384, distance='cosine')  # 必须与 EF 维度匹配
collection = client.create_collection(
    name="my_collection",
...
# 使用自定义 embedding function 创建 collection (vectors 必须手动提供)
collection = client.create_collection(
    name="my_collection",
    configuration=HNSWConfiguration(dimension=384, distance='cosine'),
    embedding_function=None  # 显式禁用 embedding function
)

# 获取或创建 collection（如果不存在则创建）
