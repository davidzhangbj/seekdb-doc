...
混合搜索（Hybrid Search）结合了基于向量的语义检索和基于全文索引的关键词检索，通过综合排序提供更准确、全面的检索结果。向量搜索擅长语义近似匹配，但对精确的关键字、数字和专有名词等匹配能力较弱，而全文搜索能有效弥补这一不足。因此，混合搜索已成为向量数据库的关键特性之一，广泛应用于各类产品中。
