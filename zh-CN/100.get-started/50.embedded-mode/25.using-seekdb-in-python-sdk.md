...
1. 连接 seekdb。
2. 创建数据库（可选）。pyseekdb 自带 test 数据库，在创建 client 的时候，不指定数据库，则默认为 test，如果想使用自定义数据库，则可以创建并指定数据库，有关创建数据库的详细操作，请参见 [create_database - 创建数据库](../../200.develop/900.sdk/50.apis/110.database/200.create-database-of-api.md)。
3. 创建带有 Embedding Functions 的 collection。
4. 使用 documents 添加数据（会自动生成 vectors）。
5. 使用 texts 进行查询（会自动生成 vectors）。
6. 打印查询结果。
...
# ==================== Step 1: 连接 seekdb，并创建自定义数据库 ====================
database_name = "my_test"
admin = pyseekdb.AdminClient(path="./seekdb.db")
admin.create_database(database_name)

# ==================== Step 2: 连接自定义数据库，创建带有 Embedding Functions 的 collection ====================
collection_name = "hybrid_search_demo"
client = pyseekdb.Client(database=database_name)
