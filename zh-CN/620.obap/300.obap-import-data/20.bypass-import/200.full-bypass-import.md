...
* 当 Session 级变量 [foreign_key_checks](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002016028) 取值为 False 时，旁路导入操作不会进行外键约束校验。
* `LOAD DATA` 语句对于带唯一索引的全量旁路导入，当出现重复唯一索引键时，暂时不支持 `REPLACE` 或 `IGNORE` 关键字，也不支持设置容错行。

## 使用 LOAD DATA 语句旁路导入数据

`LOAD DATA` 语句通过使用 `append/direct()` Hint 来执行旁路导入，在未指定 Hint 时能基于配置项 [default_load_mode](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015521) 确定导入数据的行为。
...
2. 设置导入的文件路径。
    ...
    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>由于安全原因，设置系统变量 <code>secure_file_priv</code> 时，只能通过本地 Unix Socket 连接数据库执行修改该全局变量的 SQL 语句。更多信息，请参见 <a href="https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015981">secure_file_priv</a>。</p>
    </main>
...
`INSERT INTO SELECT` 语句通过 Hint 使用 `append/direct()` 加上 `enable_parallel_dml` 来执行旁路导入，在未指定 Hint 时能基于配置项 [default_load_mode](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015521) 确定导入数据的行为。

### 使用限制

* 只支持 PDML（Parallel Data Manipulation Language，并行数据操纵语言），非 PDML 不能用旁路导入。有关并行 DML 的更多信息，参见 [并行 DML](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002013750)。
* 在导入过程中无法同时执行两个写操作语句（即不能同时写一个表），因为导入过程中会先加表锁，并且整个导入过程中只能进行读操作。
...
更多 `INSERT INTO` 语法的信息，请参见 [INSERT（MySQL 模式）](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017056) 和 [INSERT（Oracle 模式）](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017386)。
...
更多 `CREATE TABLE` 语法的信息，请参见 [CREATE TABLE（MySQL 模式）](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017069) 和 [CREATE TABLE（Oracle 模式）](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017447)。
...
* [SQL 执行计划简介](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002013759)
...
* [删除表](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015364)
