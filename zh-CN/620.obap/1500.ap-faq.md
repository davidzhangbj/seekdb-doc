...
扫描走行存时，explain 中显示的是 `TABLE FULL SCAN`，走列存时，显示的是 `COLUMN TABLE FULL SCAN`。以访问下面的 t5 表为例：
...
有关列存修改的详细信息请参见[表行存列存转换（MySQL 模式）](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015441)和[表行存列存转换（Oracle 模式）](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015372)。
...
旁路导入是一种加快数据导入，并且能够加速数据查询的数据导入方式。大表数据导入，推荐使用旁路导入方式。目前，`load data` 命令、`insert into select` 语句支持旁路导入。旁路导入的详细使用方式参见[旁路导入概述](300.obap-import-data/20.bypass-import/100.overview-of-bypass-import.md)。

### 支持使用 FlinkCDC 从其它数据库同步数据到 OceanBase 吗？

支持。参见官网文档 [使用 FlinkCDC 从 MySQL 同步数据到 OceanBase](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002012759)。
