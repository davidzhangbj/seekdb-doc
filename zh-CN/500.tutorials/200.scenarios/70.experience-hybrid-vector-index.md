...
* 联系管理员获取相应的数据库连接串，然后执行以下命令连接到数据库：
    ...
    ```
* 确保你已拥有 [AI 函数服务](../../200.develop/300.ai-function/200.ai-function.md)相关权限，并确保数据库中已使用 `CREATE_AI_MODEL` 和 `CREATE_AI_MODEL_ENDPOINT` 程序注册嵌入模型（Embedding Model）：
    :::collapse
    ```sql
...
* 了解 [稠密向量索引](../../200.develop/100.vector-search/200.vector-index/200.dense-vector-index.md)
* 深入了解 [AI 函数服务功能](../../200.develop/300.ai-function/200.ai-function.md)
* 探索 [混合搜索](../100.client-server-mode/50.experience-hybrid-search.md) 结合关键词匹配和语义理解，提供更精准、全面的搜索结果。
