...
[MCP Server](https://github.com/oceanbase/awesome-oceanbase-mcp/tree/main/src/oceanbase_mcp_server) 通过 MCP 协议提供了大模型与 seekdb 交互的能力，可以执行 SQL 语句。通过合适的客户端可以快速搭建项目原型，已在 github 上开源。
...
4. 验证是否可以连接数据库。
   ...
   Augment Code 会展示当前 test 库中的表数量，说明可以正常连接 seekdb。
