...
"""Generative AI refers to artificial intel
embedding = JinaEmbedding(
api_key=JINAAI_API_KEY,
model_type=EmbeddingModelType.JINA_EMBEDDINGS_V3)
```

### 连接到 seekdb，定义向量表结构和嵌入向量并存入 seekdb

创建一个名为 `my_seekdb_vector_table` 的表，表结构固定为 id、embedding 和 metadata。利用 Jina AI Embeddings API 为每段文本生成嵌入向量，并存入 seekdb：

```python
OB_URI = os.getenv('SEEKDB_DATABASE_HOS')
