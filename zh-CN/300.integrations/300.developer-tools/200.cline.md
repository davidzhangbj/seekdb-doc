...
在 `awesome-oceanbase-mcp` 目录下执行下面的命令创建虚拟环境，并安装依赖：
...
2. 按照下图序号指引，手动配置 seekdb MCP Server。

    ![2](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/cloud/integrations/AI/cline-6.png)

3. 填写配置文件。

    在上一个步骤中打开的 `cline_mcp_settings.json` 文件中，输入下面的配置信息，然后保存该文件。`/path/to/your/awesome-oceanbase-mcp/src/oceanbase_mcp_server` 需要替换为 `oceanbase_mcp_server` 文件夹的绝对路径，`OB_HOST`、`OB_PORT`、`OB_USER`、`OB_PASSWORD`、`OB_DATABASE` 需要替换为自己数据库的对应信息。
    ...
    "mcpServers": {
        ...
        "args": [
            "--directory", 
            "/path/to/your/awesome-oceanbase-mcp/src/oceanbase_mcp_server",
            "run",
