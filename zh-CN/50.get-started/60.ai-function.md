...
* 联系管理员获取相应的数据库连接串，然后执行以下命令连接到数据库：
    ...
    ```
* 确保你已拥有 [AI 函数服务](../700.ai/300.ai-function/200.ai-function.md)相关权限。每个示例前都会提供完整的模型和端点注册信息，您可以直接复制使用。
...
`AI_RERANK` 可以对检索结果进行智能重排序，根据查询词的相关性对文档列表重新排序。
...
<h4>注意</h4>
<p>请将所有 <code>access_key</code> 替换为实际的 API Key。如果在前面的步骤中已经注册过相应的模型，可以跳过对应的注册步骤。</p>
</main>
