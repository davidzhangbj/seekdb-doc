...
通过配置项 [audit_log_strategy](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002142567) 设置审计日志的策略，支持 `ASYNCHRONOUS`、`PERFORMANCE`、`SYNCHRONOUS` 三种类型的写日志策略，可以根据需要选择对应的写策略，以实现在性能和日志完整性之间的平衡。
...
当使用异步写日志时，可以通过配置项 [audit_log_buffer_size](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002142558) 控制 buffer 大小。

## 设置日志格式

通过设置配置项 [audit_log_format](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002142561) 选择审计日志的格式，目前只支持 CSV 格式。
...
通过配置项 [audit_log_query_sql](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002142565) 选择审计记录中的 SQL 语句，取值包含 ALL 和 NONE 两种。
...
通过配置项 [audit_log_path](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002142563) 设置审计日志的输出路径，支持本地存储和云存储。如果未指定 `audit_log_path` 的取值，则默认使用当前工作路径 `${WORK_DIR}/audit`。
...
审计记录会不断地追加写到 `audit.log` 文件中，通过设置配置项 [audit_log_rotate_on_size](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002142566) 实现自动归档日志，当 `audit.log` 的大小达到指定值时，会自动生成一个新的文件。归档只负责控制单个日志文件的大小，不负责清理文件。
...
通过配置项 [audit_log_max_size](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002142562) 和 [audit_log_prune_seconds](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002142564) 控制归档后审计日志的容量以及保存时间，两者可以同时生效。每次归档日志生成新的日志文件时会检查是否需要清理日志，日志总量在每个 OBServer 目录中单独计算。
...
通过设置配置项 [audit_log_compression](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002142559) 控制日志压缩的启用。如果开启了压缩，日志归档时会直接生成压缩后的文件，压缩算法仅支持 `ZSTD`，也支持流式压缩，且压缩后的文件使用外部工具 [zstd](https://github.com/facebook/zstd) 直接解压。
