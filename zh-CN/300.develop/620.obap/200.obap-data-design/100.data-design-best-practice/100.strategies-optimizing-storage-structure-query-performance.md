...
OceanBase 支持表数据按列存储以及行存列存冗余存储，可以根据业务场景来选择合适的数据存储格式。
...
通过租户级配置项 [default_table_store_format](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015561) 配置表的默认存储格式。
...
在 AP 场景里，通常涉及多个维度的分析查询，如果没有一个维度可以将数据进行分区且适用于各查询，而又需要将数据打散，此时可按如下方式选择分区键做 Hash 分区，尽量将用户数据均匀打散：
...
关于如何创建列存索引的操作说明，参见 [MySQL 模式下创建列存索引](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015436) 和 [Oracle 模式下创建列存索引](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015375)。
