...
tab Oracle 租户使用示例
  ...
  2. 系统租户登录，查询 `__all_virtual_processlist`，找到正在执行慢 SQL 的 session 信息（seesion_id， svr_ip, svr_port）。
      ...
      ```sql
      select id, svr_ip,  svr_port, info from __all_virtual_processlist where info like "%select%"\G
  ...
  3. 展示 session 计划详情。
      ...
      ```sql
      select dbms_xplan.display_active_session_plan(3221489189, 'all', '11.xxx.xxx.xxx', 50000);
      +--------------------------------------------------------------------------------------------------------------------+
      | COLUMN_VALUE                                                                                                       |
      +--------------------------------------------------------------------------------------------------------------------+
      | ==============================================================================================================     |
...
* 有关 DBMS_XPLAN 系统包的详细介绍，参见 [DBMS_XPLAN（MySQL 模式）](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002016328) 和 [DBMS_XPLAN（Oracle 模式）](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002016523)