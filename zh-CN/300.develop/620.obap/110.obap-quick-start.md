...
该体验以快速搭建一个可用的 OceanBase 数据库环境为例进行说明。部署的 OceanBase 数据库环境具备数据库的基本功能，可以有效地帮助您了解 OceanBase 数据库；但是该环境不具备任何高可用特性，不建议长期使用。
...
1. 获取 all-in-one 安装包。
    ...
    OceanBase 数据库单机版安装包暂未开放下载，请联系 OceanBase 获取。

2.  在安装包所在目录下执行如下命令解压安装包并安装。

    ```shell
    [admin@test001 ~]$ tar -xzf oceanbase-standalone-all-in-one-4.3.5_xxx_xxx_xxxxxxxx.el7.x86_64.tar.gz
    [admin@test001 ~]$ cd oceanbase-standalone-all-in-one/bin/
    [admin@test001 bin]$ ./install_obd.sh
...
3. 执行以下命令，快速部署 OceanBase 数据库。
    ...
    ```shell
    [admin@test001 ~]$ obd demo -c oceanbase-standalone
    ```

    `obd demo -c oceanbase-standalone` 命令默认在当前用户家目录下以最小规格部署并启动 OceanBase 数据库及相关组件（包括 OBAgent、Grafana 和 Prometheus），固定部署名为 `demo`。
...
4.  执行输出中的连接命令连接数据库。
    ...
    `obd demo -c oceanbase-standalone` 命令成功执行后会输出通过 OBClient 连接 OceanBase 数据库的命令，示例如下。
...
5. （可选）配置密码。
    ...
    使用 `obd demo -c oceanbase-standalone` 命令快速部署 OceanBase 数据库后，您可参考如下步骤为 demo 集群配置密码。
      ...
      1. 修改配置文件。
          ...
          执行上述命令打开配置文件后，在配置文件中 oceanbase-standalone 组件下添加 `root_password: xxxx`，添加完成后保存退出。示例如下：
            ...
            oceanbase-standalone:
              servers:
                - 127.0.0.1
              global:
                home_path: /home/admin/oceanbase-standalone
                ... # 省略部分配置项
                log_disk_size: 13G
                root_password: ******
            ```
