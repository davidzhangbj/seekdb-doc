...
使用 obshell Dashboard 的性能监控功能依赖 obagent 和 Prometheus 工具，告警功能依赖 Alertmanager 工具。obagent 提供 http 接口可以从 OceanBase 数据库中采集数据，Prometheus 根据配置定期访问 obagent 的接口采集监控数据并进行持久化存储，obshell Dashboard 提供页面展示 Prometheus 采集的监控数据。Alertmanager 处理来自 Prometheus 等监控系统的告警，并提供告警的去重、分组、路由和静默等功能。
...
2. 设置集群名  
   ...
   如需使用一个 Prometheus 监控多个 SeekDB 实例，需要为每个 SeekDB 实例设置不同的集群名，以便更好地区分各个 SeekDB 实例。
   ...
   <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>设置集群名时，必须为配置项 <code>cluster</code> 加上反引号（<code>`</code>）才能执行成功。</p>
   </main>
...
1. 安装 obagent  
   ...
   访问 [开源镜像库](https://mirrors.aliyun.com/oceanbase/community/stable/?spm=a2c6h.25603864.0.0.274052d2N5ZGJA)，根据所用环境的架构对应下载 obagent 安装包，并执行如下命令安装。建议安装最新版本。
...
2. 配置 obagent
   ...
   * `monagent.ob.cluster.name`：SeekDB 实例的集群名，可连接实例后执行 `SHOW PARAMETERS LIKE 'cluster';` 命令查看，`value` 列的值即为集群名。
   * `monagent.pipeline.ob.status`：配置为 `active` 表示开启 OceanBase 数据库相关指标的采集。
...
* 访问数据查询接口
  ...
  ob_server_num{app="OB",ob_cluster_id="0",ob_cluster_name="obcluster",obzone="",server_ips="172.16.81.151",status="ACTIVE",svr_ip="127.0.0.1",svr_port="2882"} 1 1762135926015
...
可访问 [Prometheus 官网](https://prometheus.io/) 或 [开源镜像库](https://mirrors.aliyun.com/oceanbase/community/stable/)，根据所用环境的架构下载对应的安装包部署 Prometheus。一般仅需要一个 Prometheus 实例即可监控所有的 OceanBase 数据库轻量版实例。本节提供配置示例仅供参考，具体安装及配置介绍可参见 [Prometheus 官网文档](https://prometheus.io/docs/prometheus/latest/getting_started/)。

obagent 提供 3 个监控请求的 URL，配置对应的采集任务，`targets` 中配置 obagent 的列表。

<main id="notice" type='explain'>
   <h4>说明</h4>
   <p>可访问 obd <a href="https://github.com/oceanbase/obdeploy/blob/master/plugins/prometheus/2.37.1/prometheus_rules.yaml">GitHub 仓库</a> 查看 OceanBase 提供的 Prometheus 告警规则文件。</p>
</main>
...
<main id="notice" type='explain'>
   <h4>说明</h4>
   <p>不同版本的 Prometheus 页面界面词不同，此处 <b>Target</b> 字段仅是示例。</p>
</main>
...
```shell
[root@test alertmanager]# ./alertmanager --config.file=alertmanager.yml --cluster.advertise-address=127.0.0.1:9093
```

示例中 `127.0.0.1:9093` 表示服务地址。
