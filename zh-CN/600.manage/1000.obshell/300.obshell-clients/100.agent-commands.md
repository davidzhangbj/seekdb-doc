...
本文介绍 obshell 的代理命令组，代理命令组可用于管理 obshell。您可通过在命令中配置 `-h`/`--help` 选项查看命令的帮助信息，如 `obshell agent start -h --seekdb`，其中 `--seekdb` 表示查看适用于 OceanBase 数据库轻量版的帮助信息。
...
```shell
obshell agent start [-P] [--password] [--seekdb] [--base-dir]
...
| --password | 否 | string | 无 | 配置 SeekDB 实例的 root 用户密码。可通过系统变量 `OB_ROOT_PASSWORD` 配置 root 用户密码。 |
...
```shell
obshell agent stop [--port] [--seekdb]
...
```shell
obshell agent restart [-P] [--password] [--seekdb]
...
| --- | --- | --- | --- | --- |
| -P/--port | 否 | int | 2886 | 设置该 obshell 绑定到的端口号。 |
| --password | 否 | string |  | 配置 SeekDB 实例的 root 用户密码。可通过系统变量 `OB_ROOT_PASSWORD` 配置 root 用户密码。 |
| --seekdb | 否 | 无 | 无 | 该选项无需传值，配置即表示命令应用于 SeekDB。 |
...
```shell
obshell agent upgrade -d [-V] [-t] [-y] [--port] [--seekdb] [-v]
