...
本文介绍 obshell 的代理命令组，代理命令组可用于管理 obshell。您可通过在命令中配置 `-h`/`--help` 选项查看命令的帮助信息，如 `obshell agent start -h --seekdb`，其中 `--seekdb` 表示查看适用于 OceanBase 数据库轻量版（SeekDB）的帮助信息。
...
```shell
obshell agent upgrade -d [-V] [-t] [--port] [--seekdb] [-y] [-v]
