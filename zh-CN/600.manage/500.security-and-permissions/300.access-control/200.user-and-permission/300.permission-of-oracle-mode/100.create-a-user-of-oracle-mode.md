...
* `PRIMARY_ZONE`：用于指定用户的 Primary Zone。对于 OceanBase 数据库单机版，只有一个 Zone。
...
2. 执行以下命令，创建用户 `test2`。
   ...
   `CREATE USER` 语句的更多信息，请参见 [CREATE USER](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017418)。
...
3. 执行以下命令，授予用户 `test2` 连接数据库的权限。
   ...
   `GRANT` 语句的更多信息，请参见 [GRANT](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017407)。
