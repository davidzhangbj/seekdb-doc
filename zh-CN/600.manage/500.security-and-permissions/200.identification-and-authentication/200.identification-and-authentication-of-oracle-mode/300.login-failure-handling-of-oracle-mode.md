...
1. 创建 Profile。
   ...
   * `integer | UNLIMITED | DEFAULT`：用于指定参数值的取值范围，其中：
   ...
   更多 `CREATE PROFILE` 语句的详细信息，参见 [CREATE PROFILE](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017414)。
...
2. 应用 Profile 至用户。
   ...
   * 在创建用户时，直接指定已创建的 Profile 文件。
     ...
     更多 `CREATE USER` 语句的详细信息，参见 [CREATE USER](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017418)。
   ...
   * 如果用户已创建，则可以通过修改用户的 Profile 文件将已创建的 Profile 文件应用至用户。
     ...
     更多 `ALTER USER` 语句的详细信息，参见 [ALTER USER](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017434)。
...
1. 使用 `SYS` 用户登录集群的 Oracle 租户。
    ...
    ```shell
    obclient -usys@oracle -h127.1.xxx.xxx -P2881 -  P******* 
    ```
...
5. 验证设置结果。
   ...
   ```shell
   $ obclient -h127.1.xxx.xxx -P2881 -usecuser@oracle -p*******;
...
更多 `ALTER PROFILE` 语句的详细信息，参见 [ALTER PROFILE](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017449)。
