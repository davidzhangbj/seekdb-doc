...
2. 执行以下语句，开启 internal 方式的透明加密。
   ...
   更多配置项 `tde_method` 的说明信息，参见 [tde_method](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015529)。
...
2. 设置表的 `progressive_merge_num` 值，给表做全量合并或者渐进合并。
    ...
    * 对表执行全量合并
      ...
      2. 手动发起一轮合并。
         ...
         手动发起合并的操作，参见 [手动触发合并](../../../../650.reference/200.system-management/500.manage-data-storage/200.merge-management/400.manually-trigger-a-merge.md)。
...
3. 完成后，您可以查看以下视图，确认所有宏块是否已完成加密。
    ...
    更多视图 `V$OB_ENCRYPTED_TABLES` 的字段及说明信息，参见 [V$OB_ENCRYPTED_TABLES](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002014084)。
