...
2. 执行以下命令，查看配置项的值。
   ...
   更多配置项的查看方法，请参见 [查看配置项](200.view-cluster-parameters.md)。
...
3. 执行以下命令，修改集群配置项的值。
   ...
   * `SCOPE` 用于指定本次配置项修改的生效范围，默认值为 `BOTH`。其中：
    ...
    ```sql
    MySQL [(none)]> ALTER SYSTEM SET syslog_level='ERROR' SCOPE = SPFILE;
    ```
    
    ```sql
    MySQL [(none)]> ALTER SYSTEM SET syslog_io_bandwidth_limit = '5M';
    ```
