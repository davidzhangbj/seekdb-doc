...
3. 通过 `SET` 修改 Session 级别变量值。
    ...
    ```shell
    MySQL [(none)]> SET ob_query_timeout = 20000000;
    ```

4. 查看修改结果。

    ```shell
    MySQL [(none)]> SHOW VARIABLES WHERE variable_name LIKE 'ob_query_timeout';
    ```
...
3. 通过 `SET` 修改 Global 级别变量值。
    ...
    ```shell
    MySQL [(none)]> SET GLOBAL ob_query_timeout = 20000000;
    ```
...
4. 查看修改结果。
    ...
    ```shell
    MySQL [(none)]> SELECT * FROM oceanbase.DBA_OB_SYS_VARIABLES WHERE NAME='ob_query_timeout';
    ```
