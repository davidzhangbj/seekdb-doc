...
在使用 AI 函数服务之前，需要确保你已拥有相关权限。详细权限请参见 [AI 函数服务权限](200.ob-vector-search-ai-function-permission.md)。

## 注意事项

* 混合检索（Hybrid Search）依赖 AI 函数服务的模型管理和嵌入（embedding）功能。在删除 AI 模型时，需检查其是否被混合检索引用，以避免潜在问题。
...
你可以通过调用该系统包，实现在 SeekDB 内部直接管理 AI 模型和端点，而不需要依赖外部服务。详细参数和语法请参考 [DBMS_AI_SERVICE](../../../700.reference/600.pl-reference/200.pl-mysql/1000.pl-system-package-mysql/1000.dbms-ai-service-mysql/100.dbms-ai-service-overview-mysql.md) 包的相关文档。
...
* [CDB/DBA_OB_AI_MODEL_ENDPOINTS](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000004018317)：监控 AI 模型调用情况。
...
<h4>说明</h4>
<p>使用 AI 函数表达式时，需要确保数据库中已注册 AI 模型及端点信息。</p>
</main>
...
|------|------|------|----------|
| model_key | 数据库内注册的模型。 | VARCHAR(128) | No |
| query | 用户输入的检索文本。 | VARCHAR(1024) | No |
...
* [向量嵌入技术](../100.vector-search/150.vector-search-generate-vector-embeddings.md)
* [MySQL 模式下的权限分类](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000003980139)