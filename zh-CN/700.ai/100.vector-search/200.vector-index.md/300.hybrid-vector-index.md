...
本文档介绍了如何在 SeekDB 中创建语义索引。

## 概述

语义索引利用 SeekDB 内置的嵌入（Embedding）能力，极大地简化了向量索引的使用流程。它实现了向量概念对用户的透明化：你可以直接写入需要存储的原始数据（如文本），SeekDB 会在内部自动将其转换为向量并建立索引。在检索时，你同样只需提供原始检索内容，SeekDB 也会自动进行嵌入并检索向量索引，从而显著提升了使用的便捷性。
...
为了进一步优化检索性能，如果你已经拥有了检索内容的向量表示，也可以直接使用这些向量来检索语义索引。例如，对于频繁重复的检索，你可以预先通过 AI 函数服务（AI Function Service）生成查询向量，从而避免每次检索都进行重复的嵌入操作。

此外，本特性还提供了对语义索引进行暴力检索的能力，用以辅助判断检索结果的正确性。暴力检索指的是采用全表扫描的方式进行检索，得到距离最近的前 n 行的精确结果。
...
本特性支持语义索引的创建、更新、删除与检索的全流程，同时适配了 `DBMS_VECTOR` 系统包内的 `REFRESH_INDEX` 和 `REBUILD_INDEX`。其中，更新、删除与检索的使用语法和常规向量索引完全一致，异步模式下 `REFRESH_INDEX` 会额外触发数据的嵌入。创建、检索的使用详见下文。
...
* [REFRESH_INDEX](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000003980772)
* [REBUILD_INDEX](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000003980771)