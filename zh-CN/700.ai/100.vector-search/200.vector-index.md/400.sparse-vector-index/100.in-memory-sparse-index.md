...
本文档介绍 SeekDB 基于内存的稀疏向量索引的创建、检索和使用方法。

## 概述

基于内存的稀疏向量索引是 SeekDB 针对稀疏向量数据（大部分元素为零的向量）提供的高效索引类型。稀疏向量索引基于内存存储，需要完整载入内存，支持 DML 和实时检索。

为了提升稀疏向量的查询性能，SeekDB 集成了 VSAG 算法库的稀疏向量索引（SINDI），该索引在性能上优于基于磁盘的稀疏向量索引，适合在资源充足的情况下使用。
...
| 检索 | 向量检索 | 支持使用 SQL 函数进行检索。 |
| 检索 | 查询参数 | 支持在检索时通过 `parameters` 子句设置查询级参数。 |
...
| `prune` | `false` | `true`/`false` | 否 | 是否对向量执行剪枝。 | `prune` 为 `true` 时，才需要设置 `refine` 和 `drop_ratio_build` 参数；`prune` 为 `false` 时，可以提供全精度的检索，如果设置 `refine` 为 `true` 或者 `drop_ratio_build` 非 `0` 会报错。 |
| `refine` | `false` | `true`/`false` | 否 | 是否需要重排。 | 取值为 `true` 时，会对检索结果获取原始稀疏向量来进行高精度的距离计算并重排，意味着需要多存储一份原始向量数据。仅在 `prune=true` 时可设置。 |
...
| `drop_ratio_search` | `0` | `[0, 0.9]` | 否 | 对检索稀疏向量数值进行剪枝的比例。 | 取值越大，剪枝越多，准确率越低，性能越高。也可以在检索时通过 `parameters` 子句设置，查询参数优先级更高。 |
| `refine_k` | `4.0` | `[1.0, 1000.0]` | 否 | 表示参与重排的比例。 | 检索 `limit_k * refine_k` 项结果，并获取原始向量进行重排。`refine=true` 的情况下才有意义。也可以在检索时通过 `parameters` 子句设置，查询参数优先级更高。 |
...
稀疏向量索引的检索语法与稠密向量索引类似，使用 `APPROXIMATE`/`APPROX` 关键字进行近似最近邻检索。
...
详细要求请参见[稠密向量索引](../200.vector-index.md)。这里仅介绍稀疏向量索引的特殊要求：
...
| c1 | c2                  | score                |
+----+---------------------+----------------------+
|  5 | {1:0.5,2:0.1,3:0.7} | -0.21000000834465027 |
|  4 | {1:0.5,2:0.2,3:0.6} | -0.18000000715255737 |
|  3 | {1:0.3,2:0.3,3:0.5} | -0.15000000596046448 |
+----+---------------------+----------------------+
3 rows in set
```
...
基于内存的稀疏向量索引提供了一些监控视图，且支持使用 `DBMS_VECTOR` 系统包进行索引维护，包括增量刷新和全量重建。详细说明请参见[创建向量索引-监控/维护](../200.vector-index.md)。

## 相关文档

* 关于稀疏向量数据类型的详细信息，请参见[向量数据类型](../../700.vector-search-reference/100.vector-data-type.md)。
* 关于向量距离函数的详细信息，请参见[向量函数](../../250.vector-function.md)。
* 关于向量索引的监控与维护，请参见[向量索引监控与维护](../200.vector-index.md)。
