...
| API 接口 | 参数描述 | 示例 |
|------|------|------|
| `def create_schema(self, **kwargs) -> CollectionSchema:` | <ul>构造一个 CollectionSchema 对象<li>可以不传参数，即初始化一个空的模式定义。</li><li>可选参数如下：</li><ul><li>fields：一个 FieldSchema 列表（详见下文 add_schema 接口）</li><li>partitions：分区规则（详见使用 ObPartition 定义分区规则章节）</li><li>description：与 Milvus 兼容用，在 SeekDB 中暂无实际作用</li></ul> |  |
