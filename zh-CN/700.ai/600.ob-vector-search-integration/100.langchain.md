...
SeekDB 提供了向量类型存储、向量索引、embedding 向量检索的能力。可以将向量化后的数据存储在 SeekDB，供下一步的检索使用。
...
本教程结合通义千问 API 演示如何将 SeekDB 中的 [向量检索功能](../../640.ob-vector-search/100.ob-vector-search-overview/100.ob-vector-search-intro.md)、[通义千问](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000001579715) 与 [LangChain](https://python.langchain.com/) 集成实现文档问答。
...
* 您可以设置 `ob_vector_memory_limit_percentage` 配置项，以启用向量检索功能。推荐保持默认值 `0`（自适应模式）。如需更精确设置此配置项，请参考相关配置文档。
...
* `$database_name`：需要访问的数据库名称。
    ...
    <main id="notice" type='notice'>
        <h4>注意</h4>
        <p>连接数据库的用户需要拥有该数据库的 <code>CREATE</code>、<code>INSERT</code>、<code>DROP</code> 和 <code>SELECT</code> 权限。更多有关用户权限的信息，请参见 <a href="../../600.manage/500.security-and-permissions/300.access-control/200.user-and-permission/200.permission-of-mysql-mode/100.permission-classification-of-mysql.md">MySQL 模式下的权限分类</a>。</p>
    </main>

* `$user_name`：提供数据库连接账户，格式：`用户名`。
* `$password`：提供账户密码。

更多连接串的信息，请参见 [通过 OBClient 连接 SeekDB](../../300.develop/100.application-development-of-mysql-mode/100.connect-to-oceanbase-database-of-mysql-mode/300.connect-to-an-oceanbase-tenant-by-using-obclient-of-mysql-mode.md)。
