...
本文介绍了 OceanBase 如何创建、检索、维护和删除向量索引。
...
支持通过 `DBMS_VECTOR` 系统包进行索引内存估算： // TODO 看下系统包放哪
...
增量数据过多的情况下，检索性能会下降。为减小增量数据表的数据量，OceanBase 引入了 `DBMS_VECTOR` 对向量索引进行维护。
...
OceanBase 向量检索为 IVF 索引提供了自动性能优化机制，通过缓存管理和定期维护来提升检索性能。
