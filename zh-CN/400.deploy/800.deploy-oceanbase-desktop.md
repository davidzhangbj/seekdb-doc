...
桌面版 SeekDB 是一个用于管理和操作 OceanBase 数据库的桌面应用程序。它提供了图形化界面，使用户能够方便地进行数据库管理、查询执行、数据导入导出等操作。
...
3. 启动桌面版 SeekDB 
   ...
   安装完成后会自动部署一个处于 `已停止` 状态的 OceanBase 数据库，单击桌面版 SeekDB 中的 **启动** 按钮即可启动 OceanBase 数据库。
...
OceanBase 数据库启动成功后，单击桌面版 SeekDB 中的 **连接** 按钮，即可自动连接 OceanBase 数据库。您也可手动执行连接 OceanBase 数据库，连接串示例如下：
