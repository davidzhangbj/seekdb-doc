...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>您必须在每台部署 OceanBase 节点的机器上执行此操作。</p>
</main>
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <ul><li>您需查看 <code>/etc/security/limits.d/20-nproc.conf</code> 文件中是否存在 nproc 的配置，若存在需同步修改该文件中 nproc 的值。</li>
  <li>limits.conf 的配置仅对新登录的会话生效（如新打开的终端、SSH 连接或新启动的进程）。当前已存在的会话或进程不会自动继承新配置，需要重新登录或重启相关服务。</li></ul>
</main>
