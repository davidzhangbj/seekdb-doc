...
1. 您可从 [OceanBase 软件下载中心](https://www.oceanbase.com/softwarecenter) 下载所需版本的 SeekDB 及对应依赖库（`OceanBase Libs`）。下载完成后，将安装包复制到您的机器上。推荐您使用最新版本的安装包。

2. 在安装包所在目录下，执行 rpm 命令安装 SeekDB。

   ```shell
   [admin@test001 ~]$ sudo rpm -ivh oceanbase-*.rpm
   ```
...
2. 修改 SeekDB 安装目录（默认是 `/home/admin/oceanbase`）下 `etc` 目录的属主
...
* -u：提供 SeekDB 的连接账户，格式：`用户名@租户名`。MySQL 租户的管理员用户名默认是 `root`。

* -P：提供 SeekDB 连接端口，为 `/etc/oceanbase.cnf` 中 `mysqlPort` 的值。

* -p：提供 SeekDB 连接密码，为 `/etc/oceanbase.cnf` 中 `rootPwd` 的值。建议不在连接命令中提供，改为在执行命令后的提示符下输入，密码文本不可见。
...
当 SeekDB 状态出现异常时，您可执行如下命令查看 SeekDB 的详细信息。

```shell
[admin@test001 ~]$ sudo journalctl -u oceanbase
```

Systemd 实际上是通过发送请求与 SeekDB 的管控工具 obshell 进行通信，从而来实际控制 OBServer 节点的状态。在日志中，可以看到很多 trace id，可以通过 obshell 的接口和 trace id 结合排错，命令如下：
...
2. 卸载 SeekDB 
   ...
   2. （可选）删除 SeekDB 数据
      ...
      执行第一步后，您环境中的 SeekDB 已被卸载，但是 SeekDB 的数据还存在。若您之后重新部署 SeekDB，还可以查看和操作该 SeekDB 的数据。
