...
* -P：提供 SeekDB 连接端口，为 `/etc/oceanbase/seekdb.cnf` 中 `mysqlPort` 的值。

* -p：提供 SeekDB 连接密码，为 `/etc/oceanbase/seekdb.cnf` 中 `rootPwd` 的值。建议不在连接命令中提供，改为在执行命令后的提示符下输入，密码文本不可见。
