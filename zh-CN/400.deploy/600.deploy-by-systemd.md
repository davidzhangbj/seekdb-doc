...
* 您使用的环境是 RPM 平台系统，目前已验证支持的系统如下。
  ...
  * Ubuntu 20.04、22.04 和 23.04 版本（内核 Linux 4.19 版本及以上）
...
1. （可选）修改配置文件
   ...
   | datafile_next  | 可选      | 0        | 指定数据文件增长的步长。 |
   | datafile_maxsize  | 可选      | 0        | 指定数据文件的最大大小。 |
...
* `Active` 展示为 `active (running)`，且 `Status` 展示为 `observer is ready and running`，表示已成功启动 SeekDB。
