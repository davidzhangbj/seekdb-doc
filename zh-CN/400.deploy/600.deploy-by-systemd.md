...
* 您使用的环境是 RPM 平台系统，目前已验证支持的系统如下。
  ...
  * CentOS Linux 7.x 、8.X 版本（内核 Linux 4.19 版本及以上）
...
2. 在安装包所在目录下，执行 rpm 命令安装 SeekDB。
   ...
   ```shell
   [admin@test001 ~]$ sudo rpm -ivh oceanbase-seekdb-*.rpm
   ```
...
1. （可选）修改配置文件
   ...
   ```shell
   [admin@test001 ~]$ sudo vim /etc/oceanbase/oceanbase.cnf
   ```
   ...
   |  配置项  |  是否必选  |  默认值  |  说明     |
   |----------|-----------|----------|----------|
   | mysql_port | 可选     | 2881    | SeekDB 的 SQL 服务协议端口号。  |
   | root_pwd  | 可选      | 默认为空       | SeekDB 的用户（`root`）的密码，建议设置复杂的密码。 |
   ...
   | datafile_size  | 可选      | 0        | 指定数据文件（block_file）大小，未配置的情况下以 `datafile_disk_percentage` 配置项为准，详细介绍请参见 [datafile_size](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015725) 和 [datafile_disk_percentage](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015736)。 |
   | cpu_count  | 可选   | 0  |  SeekDB 可使用的 CPU 总数，如果设置为 0，将自动检测。   |
   | memory_limit  | 可选      | 0       | SeekDB 能从环境中获取的最大内存，未配置的情况下以 `memory_limit_percentage` 配置项为准，配置项详细介绍请参考 [memory_limit](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015826) 和 [memory_limit_percentage](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015788)。 |
   | system_memory  | 可选      | 0M        | 保留的系统内存，该参数值会占用 `memory_limit` 的内存，未配置的情况下 SeekDB 会自适应。 |
   | log_disk_size  | 可选      | 0        | 用于设置 Redo 日志磁盘的大小，未配置的情况下以`log_disk_percentage` 配置项为准，详细介绍请参考 [log_disk_size](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015772) 和 [log_disk_percentage](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015630)。|
...
* -P：提供 SeekDB 连接端口，为 `/etc/oceanbase/oceanbase.cnf` 中 `mysqlPort` 的值。

* -p：提供 SeekDB 连接密码，为 `/etc/oceanbase/oceanbase.cnf` 中 `rootPwd` 的值。建议不在连接命令中提供，改为在执行命令后的提示符下输入，密码文本不可见。
...
* `Active` 展示为 `active (running)`，且 `Status` 展示为 `Service is running` 时，表示 SeekDB 正在启动中。

* `Active` 展示为 `active (running)`，且 `Status` 展示为 `Service is ready` 时，表示已成功启动 SeekDB。
...
2. 卸载 SeekDB 
   ...
   2. 删除 SeekDB 数据
      ...
      执行第一步后，在返回结果中会显示 `To clean OceanBase : bash /var/lib/oceanbase/oceanbase_clean.sh`，可以通过该命令，完全

      执行本步骤删除 SeekDB 数据后，您部署的 SeekDB 将完全被卸载。

      ```shell
      [admin@test001 ~]$ bash /var/lib/oceanbase/oceanbase_clean.sh
      ```
