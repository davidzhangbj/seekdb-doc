...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>如果需要使用 HDFS/ODPS 外表功能，需要在对应机器节点进行如下的部署配置 JAVA 环境和安装依赖。</p>
</main>
...
2. 通过正常解压缩对应的安装包即可。
    ...
    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>该路径用于配置当前 OBServer 运行节点上的 java home 目录，即对应配置项 <a href="https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002525417">ob_java_home</a> 的取值。</p>
    </main>
...
1. 获取 devdeps-hdfs-sdk RPM 安装包。
   ...
   请联系技术支持获取 devdeps-hdfs-sdk RPM 安装包。
...
1. 获取 devdeps-java-extensions RPM 安装包。
    ...
    请联系技术支持获取 devdeps-java-extensions RPM 安装包。

      <main id="notice" type='notice'>
        <h4>注意</h4>
        <p>请下载 1.0.1 版本 devdeps-java-extensions RPM 安装包。</p>
      </main>
...
3. 检查对应安装是否符合预期。
    ...
    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>该路径用于配置 OBServer 启动的 jvm 可加载的可执行依赖 jar 包路径，即对应配置项 <a href="https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002525418">ob_java_connector_path</a> 的取值。</p>
    </main>
...
1. 开启 java env，用于外表的 sdk（java sdk）访问。
    ...
    **示例如下：**
    ...
    该设置的详细介绍信息，参见 [ob_enable_java_env](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002525416)。
...
2. 设置当前 OBServer 运行节点上的 java home 目录。
    ...
    **示例如下：**
    ...
    该设置的详细介绍信息，参见 [ob_java_home](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002525417)。
...
3. 设置 OBServer 启动的 jvm 可加载的可执行依赖 jar 包路径。
    ...
    **示例如下：**
    ...
    该设置的详细介绍信息，参见 [ob_java_connector_path](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002525418)。
...
4. 设置 java 环境启动的相关配置项
   ...
   2. 设置 java 运行的 jvm 启动配置项。
       ...
       **示例如下：**
       ...
       该设置的详细介绍信息，参见 [ob_java_opts](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002525419)。
...
5. （可选）设置 ODPS 使用 JAVA SDK 配置项。
    ...
    如需使用 JAVA SDK，需要设置 <code>_use_odps_jni_connector<code> 为 <code>true</code>。
...
停止所有机器中的 observer 进程，然后重新启动 observer 进程。
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>如果集群是通过 obd 部署，则可以使用 obd 命令重启；如果是通过 OCP 部署，则使用 OCP 进行重启。</p>
  </main>
...
2. 重新启动 observer 进程。
    ...
    ```shell
    -bash-4.2$ obd cluster restart [cluster_name]
    ```


## 相关文档

有关创建 ODPS 外表的详细介绍信息，参见 [创建外表（MySQL 模式）](../650.reference/300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/1000.manage-external-tables-of-mysql-mode/200.create-a-external-table-of-mysql-mode.md) 或 [创建外表（Oracle 模式）](../650.reference/300.database-object-management/200.manage-object-of-oracle-mode/100.manage-tables-of-oracle-mode/1000.manage-external-tables-of-oracle-mode/200.create-a-external-table-of-oracle-mode.md)
