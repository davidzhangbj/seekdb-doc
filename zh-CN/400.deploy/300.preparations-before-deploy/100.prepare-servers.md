...
本节主要介绍不同的部署方式和部署形态时，所需要的服务器数量和规格以及服务器的软硬件要求，包括操作系统、BIOS 设置、网卡设置等。
...
|---------|-----------|-------------|-------------|------|
| OceanBase 单机版数据库 | 1 台  | 最低要求：CPU 1 核，可用内存 2G |  SSD 存储 |

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p><ul>
    <li>磁盘中日志盘需要内存的 3 倍以上，数据盘要满足目标数据量的存储。</li>
    <li>OBServer 服务器内存设置建议不超过 1T。</li>
    <li>这里列的资源需求指的是为 OceanBase 所预留的资源，而不是服务器本身的全部资源。</li>
  </ul></p>
</main>
...
查看服务器内核信息，确保操作系统为 3.10.0 及以上，同时根据内核信息，选择对应的 all-in-one 部署安装包。
