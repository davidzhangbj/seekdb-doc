...
|------------|---------|-----------|-----------|
| 第一层 | Shards | Partition | Milvus 通过在模式定义时设置某些列为 `partition_key` 来制定分区规则<br/>seekdb 支持 range/range columns、list/list columns、hash、key 以及二级分区策略 |
| 第二层 | Partitions | ≈Tablet | Milvus 支持在同一个 Shards 中（Shards 通常按照主键进行分区）按照其他列进行分块，以提升读性能<br/>seekdb 则是通过在分区内按照主键排序实现 |
...
pyobvector 具体接口使用文档请参见 [pyobvector Python SDK 接口说明](900.vector-search-supported-clients-and-languages/200.vector-pyobvector.md)。
...
| JSON | JSON | JSON 操作异同点请参见 [pyobvector Python SDK 接口说明](900.vector-search-supported-clients-and-languages/200.vector-pyobvector.md) |
| FLOAT_VECTOR | VECTOR | 向量类型 |
| BINARY_VECTOR | 不支持 |  |
| FLOAT16_VECTOR | 不支持 |  |
| BFLOAT16_VECTOR | 不支持 |  |
| SPARSE_FLOAT_VECTOR | 不支持 |  |
| dynamic_field | 无需支持 | Milvus 中的隐藏 `$meta` 元数据列<br/>seekdb 显示创建 JSON 类型的列即可 |
...
* Segments 类似于 [MemTable](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000003977777) + [SSTable](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000003977776)。
...
1. Milvus search()：
    ...
    * filter：str 表达式，具体使用示例参见：[milvus 的过滤说明](https://milvus.io/docs/boolean.md#Usage)，大体上和 SQL 的 `WHERE` 表达式类似。
