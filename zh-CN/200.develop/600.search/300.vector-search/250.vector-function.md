...
* 稠密向量索引支持 L2、内积（Inner Product）、余弦距离作为索引距离算法；基于内存的稀疏向量索引支持内积作为距离算法。具体请参见[稠密向量索引](200.vector-index/200.dense-vector-index.md)和[基于内存的稀疏向量索引](200.vector-index/400.sparse-vector-index/100.in-memory-sparse-vector-index.md)。
