...
* 稠密向量索引支持 L2、内积（Inner Product）、余弦距离作为索引距离算法；基于内存的稀疏向量索引支持内积作为距离算法。具体请参见[创建向量索引](200.vector-index/200.vector-index.md)和[稀疏向量索引](200.vector-index/400.sparse-vector-index/100.in-memory-sparse-index.md)。
* 向量索引检索支持调用本文档中的 `L2_distance`、`Cosine_distance`、`Inner_product` 距离函数。
