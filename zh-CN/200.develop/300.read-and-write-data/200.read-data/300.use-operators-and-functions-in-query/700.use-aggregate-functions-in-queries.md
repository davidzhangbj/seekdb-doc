...
在 seekdb 中，聚合函数跟 `GROUP BY` 子句一起使用时，对 `select_list` 中的列没有特殊要求。如果要求 `select_list` 里的列跟 `GROUP BY` 子句中的列保持一致，需要设置 MySQL 命令行下的 `sql_mode` 为 `ONLY_FULL_GROUP_BY`。示例如下：
...
有关seekdb 当前版本所支持的聚合函数详情，请参见 **《SQL 参考》** 章节。
