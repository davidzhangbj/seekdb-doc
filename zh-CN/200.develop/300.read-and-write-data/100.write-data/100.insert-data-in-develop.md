...
* 请确认您已连接到 seekdb，连接数据库的操作请参见 [连接方式概述](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002013253)。

* 请确认您已拥有待操作的表的 `INSERT` 权限，查看当前用户权限的相关操作请参见 [查看用户权限](../../../400.guides/500.manage/300.security/300.access-control/200.user-and-permission/400.view-user-permissions.md)。如果不具备该权限，请联系管理员为您授权，用户授权的相关操作请参见 [直接授予权限](../../../400.guides/500.manage/300.security/300.access-control/200.user-and-permission/200.direct-authority.md)。

## 使用 INSERT INTO 语句插入数据

请使用 [INSERT](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017056) 语句，再参考下面的建议，向表中插入数据。
...
* 通过  [`INSERT IGNORE INTO`](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017056) 语句避免约束冲突时，`IGNORE` 关键字可以忽略由于约束冲突导致的 `INSERT` 失败的影响。
...
* 通过 [`INSERT INTO ON DUPLICATE KEY UPDATE`](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017056) 语句避免约束冲突时，可以指定对重复主键或唯一键的后续处理。
...
| select_stmt | 指定 `SELECT` 子句。有关查询语句的详细信息，参见 [SELECT 语句](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017573)。|
...
* 该语句数据写入操作是全量旁路导入方式，所以操作受全量旁路导入功能限制。有关旁路导入的信息，参见 [使用 INSERT INTO SELECT 语句旁路导入数据](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002012783) 中的 **使用限制** 章节。
* 该语句指定旁路导入 Hint 会报错。
* 受 PDML（Parallel Data Manipulation Language，并行数据操纵语言）框架限制，PDML 不支持的场景无法导入数据，`INSERT OVERWRITE SELECT` 会报错 not supported。有关并行 DML 的详细信息，参见 [并行 DML](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002013750)。
...
* 分区级的 `insert overwrite` 操作支持目标表最后一级分区为 Hash 或 Key 分区。
...
除了 `INSERT` 语句，当表中无数据记录，或者表中有数据记录但无主键或唯一键冲突时，还可以使用 `REPLACE INTO` 语句代替 `INSERT` 语句插入数据。`REPLACE INTO` 语句的详细语法及说明请参见 [REPLACE](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017054)。
...
* [更新数据](200.update-data-in-develop.md)
  
* [删除数据](300.delete-data-in-develop.md)

* [替换数据](400.replace-data-in-develop.md)
