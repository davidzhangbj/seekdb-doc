...
# Druid 连接池连接 seekdb 示例程序

本文将介绍如何使用 Druid 连接池、MySQL Connector/J 和 seekdb 构建一个应用程序，实现基本的数据库操作，包括创建表、插入数据、更新数据、删除数据、查询数据和删除表等。

[点击下载 druid-mysql-client 示例工程](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/connection-pool/druid-mysql-client/druid-mysql-client.zip)
...
根据 **步骤二：获取 seekdb URL** 中获取的信息修改 `druid-mysql-client/src/main/resources/db.properties` 文件中的数据库连接信息。
