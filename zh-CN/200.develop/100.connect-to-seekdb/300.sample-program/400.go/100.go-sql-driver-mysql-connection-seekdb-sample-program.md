...
# Go-SQL-Driver/MySQL 连接 seekdb 示例程序

本文将介绍如何使用 Go-SQL-Driver/MySQL 驱动和 seekdb 构建一个应用程序，实现创建表、插入数据和查询数据等基本操作。

[点击下载 go-oceanbase 示例工程](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/demo/Go/go-oceanbase.zip)
...
1. 安装 Go 语言
    ...
    3. 配置环境变量：将 Go 语言的安装路径添加到系统的 PATH 环境变量中。
       ...
       * 在 Linux 或 macOS 环境中，可以编辑 `~/.bashrc` 或 `~/.bash_profile` 文件，在其中添加以下内容：
        ...
        `\usr\local\go\bin`为默认安装目录，若在安装 Go 语言时修改了安装目录，请替换为对应的目录。
...
根据 **步骤二：获取 seekdb 连接信息** 中的信息修改 `test.go` 文件中的数据库连接信息。选中 `test.go` 文件，右键单击选择**打开方式**，可通过**记事本**或其他编辑软件打开。
...
`test.go` 文件定义了如何使用 Go 语言和 seekdb 的 MySQL 模式进行交互，包括连接数据库、创建表、插入数据、查询数据和删除表等操作。
`test.go` 文件包含以下内容：
...
4. 定义 `selectAll()` 函数。
    ...
    4. 插入数据。
       插入一行数据到表 `t1` 中，设置该行数据的 `str` 字段的值为 `Hello seekdb`。
...
有关 Go-SQL-Driver/MySQL 的更多信息，参见 [Go-SQL-Driver/MySQL](https://github.com/go-sql-driver/mysql#installation)。
