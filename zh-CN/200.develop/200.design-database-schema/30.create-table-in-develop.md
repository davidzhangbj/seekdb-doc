...
更多有关 seekdb 表的详细信息，请参见 [表概述](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002016766)。
...
您可以使用 `SHOW TABLES;` 语句查看数据库中表的信息。
...
* 根据数据类型的特点，为待存储在列中的数据选择合适的数据类型。
  ...
  seekdb 中支持的数据类型及详细介绍请参见 [数据类型概述](../../450.reference/200.sql/100.sql-syntax/100.basic-elements/100.data-type/100.data-type-overview.md)。
...
* 建议为表定义一个主键，每个数据库表最多拥有一个主键列集合。
  ...
  在 seekdb 中，不强制用户为表定义主键，但使用主键可以使表内的每行数据被唯一确定，且不存在重复的数据行。如果没有合适的字段作为主键，可以在创建表时不指定主键，待表创建成功后系统会为无主键表指定自增列作为隐藏主键。有关自增列的详细介绍请参见 [定义自增列](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015440)。

  此外，如果在创建时未定义主键列，seekdb 支持为已存在的表添加主键列。
...
更多主键约束相关的介绍，请参见 [主键约束](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017305)。
...
定义主键列后，支持删除主键，更多主键约束相关的操作请参见 [定义列的约束类型](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015447)。

### 定义其他列约束

除了 `PRIMARY KEY` 约束外，seekdb 还支持 `NOT NULL` 约束、唯一性（UNIQUE）约束、外键（FOREIGN KEY）约束和 `CHECK` 约束。使用约束可以简化表查询、提高查询性能并确保数据在语义上保持有效。
...
* 外键约束（`FOREIGN KEY`）：要求约束的列的值取自于另外一个表的主键列。
  ...
  seekdb 默认开启了外键约束检查，外键约束检查开关由租户变量 `foreign_key_checks` 来控制。`foreign_key_checks` 变量的更多说明，请参见 [foreign_key_checks](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002016028)。
...
定义其他列约束的详细操作请参见 [定义列的约束类型](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015447)。

### 定义表的自增列

在 seekdb 中，如果创建表时需要某个数值列的值不重复并且保持递增，可以将表中列的类型定义为 `AUTO_INCREMENT`，即自增列。
...
您可以根据业务需要，修改这三个系统变量的值，修改系统变量的操作请参见 [配置管理介绍](../../400.guides/500.manage/100.configuration-management/100.configuration-management-introduction.md)。
...
* 创建分区表时，若使用自增列作为分区键，则在 seekdb 中，自增列的值全局唯一，但在分区内不保证递增。
...
更多自增列的介绍和详细操作请参见 [定义自增列](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015440)。

### 选择分区方案

在创建表时，需要明确表的分区方案，如果表中的数据量很大时，建议对表进行分区。创建分区表时，需要根据待存储在表中的数据来选择合适的分区方式。

seekdb 中，单个表最多支持创建的分区个数为由配置项 [max_partition_num](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015551) 控制，默认为 8192 个。

在 seekdb 中，按照分区策略，分区表可以分为：
...
按照分区维度，分区表又可以分为一级分区和二级分区，二级分区是一级分区的二次拆分，因此一级分区表有一个分区键，二级分区表有两个分区键，并且两次拆分的策略可以不一样。在 seekdb 中，二级分区表包括模板化二级分区表和非模板化二级分区表。

关于分区相关的详细介绍请参见 [分区概述](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002016759)。
...
Range 分区、Range Columns 分区、List 分区、List Columns 分区、Hash 分区 和 key 分区均可以作为组合分区表的二级分区策略。在 seekdb 中，二级分区表包括模板化二级分区表和非模板化二级分区表。
...
* 创建表后，可以使用 `INSERT` 语句将数据插入到表中。更多有关插入数据的详细信息，请参见 [插入数据](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002013243)。

* 如果需要提高查询性能，可以为表中的列创建索引。更多有关创建索引的详细信息，请参见 [创建索引](40.create-index-in-develop.md)。
