...
本文介绍基于 seekdb 进行 TPC-H 测试时，所需的软件要求，以及详细的测试方法。
...
* seekdb：详细信息，请参考 [通过 yum install 部署 seekdb](../../400.deploy/700.server-mode/100.deploy-by-systemd.md)。
...
```sql
ALTER TENANT sys SET VARIABLES secure_file_priv ='/' ;
...
1. 编写测试脚本 `tpch.sh`。
   ...
   function clear_kvcache
   {
      if [[ ${password_sys} == "" ]];then
         mysql_sys="mysql -h${host} -P${port} -uroot -Doceanbase -A -c"
      else
         mysql_sys="mysql -h${host} -P${port} -uroot -Doceanbase -p${password_sys} -A -c"
      fi
      tenant_name=${user#*@}
      echo "alter system flush kvcache ;" | ${mysql_sys}
      echo "alter system flush kvcache tenant '${tenant_name}' cache 'user_row_cache';" | ${mysql_sys}
      sleep 3s
   }
