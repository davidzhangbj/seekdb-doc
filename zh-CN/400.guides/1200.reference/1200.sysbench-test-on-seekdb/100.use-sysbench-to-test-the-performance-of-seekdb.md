...
* `DBMS_MONITOR.OB_TENANT_TRACE_DISABLE`：禁用当前租户的全链路追踪功能。
...
3. 执行性能测试。
   ...
   #write_only
   sysbench --mysql-host=xxx.xxx.xxx.xxx --mysql-port=2881 --mysql-user=root --mysql-db=test --report-interval=1 --threads=400 --db-ps-mode=disable --percentile=99 --tables=30 --mysql-ignore-errors=1062 --table_size=10000 --auto_inc=on --warmup-time=2 --time=10 --rand-seez'zd=11972 --rand-type=uniform oltp_write_only.lua run
   ```
