...
seekdb 支持通过 `LOAD DATA` 语句将外部文件的数据导入到数据库表中，并提供多种使用方式。本文档介绍服务端导入和客户端导入两种方式。
...
seekdb 的 `LOAD DATA FROM FILES()` 语句支持指定错误诊断模式。允许将失败的行记录下来，而不是让整个操作因第一个错误而终止。
...
上述语句会：
* 将错误日志写入 `/home/admin/error_logs/` 目录。
* 允许最多 10 行数据导入失败，超过限制后整个导入操作才会终止。
* 将失败的数据行保存到 `/home/admin/bad_files/` 目录。

**LOG ERRORS 子句说明：**

* `LOG ERRORS`：表示开启导入过程中的错误诊断，允许将失败的行记录下来，而不是让整个操作因第一个错误而终止。

* `INTO 'logfile_string'`：可选项，用于设置错误信息写入目标目录中的文件。当不指定 `INTO 'logfile_string'` 时，错误信息只记录于 `warning buffer` 中，可以通过 `SHOW WARNINGS` 查看。`logfile_string` 表示存放错误信息的目录，支持本地路径和对象存储路径（格式同 `LOCATION` 子句）。
...
* `BADFILE 'badfile_string'`：指定错误数据文件存放的路径。`badfile_string` 取值格式同 `INTO 'logfile_string'` 的格式。

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p><ul><li>不指定 <code>LOG ERRORS</code> 子句行为为正常导入行为，即遇到第一个错误就直接报错。</li><li>指定了 <code>LOG ERRORS</code> 子句但是没有指定 <code>REJECT LIMIT</code> 子句，相当于指定了 <code>REJECT LIMIT</code> 为 0 的诊断，会在遇到第一个错误时操作失败，但是会记录遇到的第一个错误，并且错误码也是诊断相关错误，即 "reject limit reached"。</li></ul></p>
</main>
...
`READ_ERROR_LOG()` 函数支持两种使用方式：

* 指定的 log 路径为文件名时（不以 `/` 结尾），文件名会被作为 pattern 来过滤文件：

  ```sql
  SELECT * FROM READ_ERROR_LOG('diagnosis/log/path/filename');
  ```

* 指定的 log 路径为文件夹时（以 `/` 结尾），会读取目录下的所有错误日志文件：
