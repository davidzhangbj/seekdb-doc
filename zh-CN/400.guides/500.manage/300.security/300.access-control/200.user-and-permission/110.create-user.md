...
由于 `CREATE USER` 权限较大，默认仅管理员拥有此系统权限，其他用户如果需要创建用户，则需要被授予 `CREATE USER` 权限，授权相关操作请参见 [直接授予权限](200.authority-of-mysql-mode.md)。

## 用户名称的规则

为用户指定名称时，需要注意以下限制：

* 用户名的唯一性：每个用户名需要保证在 seekdb 内唯一。
  
* 用户名的命名约定：

  * 使用客户端创建用户时，要求用户名长度不超过 64 个字节。
