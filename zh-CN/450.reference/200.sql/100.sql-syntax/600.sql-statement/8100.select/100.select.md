...
* `FOR UPDATE [opt_for_update_wait]`：表示对查询结果所有行加上排他锁，以阻止其他事务的并发修改，或阻止在某些事务隔离级别时的并发读取。有关使用排他锁的介绍信息，参见 [锁定查询结果 SELECT FOR UPDATE](../../../../../../300.develop/100.application-development/500.read-data/300.use-operators-and-functions-in-query/1000.lock-query-results-select-for-update.md)。
...
* `LOCK IN SHARE MODE`：用于在查询数据时获取共享锁，防止其他事务对数据进行写操作，但允许其他事务对数据进行读操作。有关使用共享锁的介绍信息，参见 [锁定查询结果 LOCK IN SHARE MODE](../../../../../../300.develop/100.application-development/500.read-data/300.use-operators-and-functions-in-query/1100.query-results-lock-in-share-mode.md)。
...
[单表查询](../../../../../../200.develop/100.application-development/500.read-data/100.single-table-query.md)