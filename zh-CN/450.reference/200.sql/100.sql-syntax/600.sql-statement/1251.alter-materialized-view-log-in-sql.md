...
执行 `ALTER MATERIALIZED VIEW LOG` 语句，需要当前用户拥有基表（普通表或物化视图）的 `ALTER` 权限。有关 seekdb 权限的详细介绍，参见 [seekdb 的权限分类](../../../../../../../400.guides/500.manage/300.security/300.access-control/200.user-and-permission/100.permission-classification.md)。
...
* `PURGE [[START WITH expr] [NEXT expr]]`：用于修改物化视图日志的后台清理任务的时间周期。
  ...
  * `[NEXT expr]`：可选项，表示下一次清除物化视图日志的时间。用于设置下一次清除物化视图日志的时间。
  ...
  :::tip
  请确保 `START WITH expr` 和 `NEXT expr` 这两个时间表达式设定在未来的时间点，否则会引发错误。
  :::
...
* `LOB_INROW_THRESHOLD [=] integer`：用于修改物化视图日志的 LOB 内联存储长度阈值，且只支持往大调整。
  ...
  :::info
  `LOB_INROW_THRESHOLD` 默认值为 8KB。mlog 的 `LOB_INROW_THRESHOLD` 默认值不受系统变量 [ob_default_lob_inrow_threshold](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015946) 控制。
  :::
...
[物化视图日志](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002016844)