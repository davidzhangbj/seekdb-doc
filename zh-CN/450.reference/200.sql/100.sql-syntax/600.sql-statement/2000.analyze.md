...
|----------------------------|----------|
| table_name |指定要分析的表。|
| INT_NUM | 整数，即 Buckets 的个数。有关 Buckets 的取值，请参见 [method_opt](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017252)。|
...
* 当配置项 `enable_sql_extension` 为 `TRUE` 的时候，使用以下语法收集用户 `user01` 的表 `test` 的统计信息，所有列的桶个数设定为 128。
