...
创建几何值函数用于以各种格式（如 Well-Known Text (WKT) 或 Well-Known Binary (WKB)）创建几何图形。seekdb 当前版本所支持的创建几何值的函数包括 `ST_GeomFromText()` 和 `ST_GeometryFromText()`、`ST_GeomFromWKB()` 和 `ST_GeometryFromWKB()`、`_ST_MakePoint`。
...
`ST_GeomFromText()` 和 `ST_GeometryFromText()` 函数将 Well-Known Text（WKT） 表示形式和可选的空间参考系统标识符 (SRID) 作为参数，返回相应的几何图形。有关 WKT 格式的说明，请参见 [空间数据格式](../../100.basic-elements/100.data-type/900.spatial-data-type/500.spatial-data-format-of-syntax-of-syntax.md)。
...
`ST_GeomFromWKB()` 和  `ST_GeometryFromWKB()` 函数将包含 Well-Known Binary (WKB) 表示和可选的空间参考系统标识符 (SRID) 的 `BLOB` 作为参数，返回相应的几何图形。有关 WKB 格式的说明，请参见 [空间数据格式](../../100.basic-elements/100.data-type/900.spatial-data-type/500.spatial-data-format-of-syntax-of-syntax.md)。
