...
| behavior_ctrl | JSON 形式的参数指定，可选配置项如下：<ul><li>`stopwords`：停词表，如果无，按全局默认配置，应用全局停词；如果为空，则为空。</li><li>`case`：如果无，则按系统默认行为处理；如果有，按 upper/lower 进行处理。</li><li>`output`：有以下格式：<ul><li>`default`：输出仅包含词元的 `json array, ["hello", "world", "english"]`。</li><li>`all`：包含 `doc_len` 和词频的 `json object`，如 `{"doc_len":3, "tokens":["i":1, "love":1, "china":1]}`。</li><li>`additional-args`：["token_size:2"]} 特定分词器的参数，如`ngram`。</li></li></ul>|
