...
* `FIRST | BEFORE | AFTER`：指定待添加列的位置，可以添加在表首（`FIRST`）、 `column_name` 之前（`BEFORE`）或 `column_name` 之后（`AFTER`）。
  ...
  使用 `FIRST` 将列添加到表首时，无需设置 `column_name`，否则会执行报错。 </p>
...
seekdb 相关的列类型变更规则，请参考 [列类型变更规则](../700.ddl-function/900.column-type-change-rule.md)。
...
[ALTER TABLE](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017015)