...
1. 启用半结构化编码
   ...
   如果启用半结构化编码功能，请务必保证配置项 [micro_block_merge_verify_level](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002015689) 为默认配置 `2`，切勿关闭微块合并校验。
...
使用半结构化编码可以提高 [JSON_VALUE() 函数](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002018125) 条件过滤查询的性能。基于 JSON 半结构化编码技术，seekdb 对 `JSON_VALUE` 表达式条件过滤查询场景进行了性能优化。由于 JSON 数据已被拆分为子列，系统可以直接基于编码后的子列数据进行过滤，无需还原完整的 JSON 结构，从而显著提升查询效率。
