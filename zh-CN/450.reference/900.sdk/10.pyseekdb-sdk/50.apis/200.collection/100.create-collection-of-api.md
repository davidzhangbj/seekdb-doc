...
|`configuration`|Configuration \| HNSWConfiguration|可选|具有维度和距离度量的索引配置。 如果未提供，则使用默认值，默认为 `dimension=384, distance='cosine',analyzer=’ik’`。如果设置为 `None`，则维度将从 `embedding_function` 值计算，如果 `embedding_function` 也为 None，则会引发错误。|HNSWConfiguration(dimension=384, distance='cosine')|
...
其中有关参数 `configuration` 中 `Configuration` 类型的参数说明如下：

|参数|取值类型|是否必选|描述|取值示例|
|---|---|---|---|---|
|`hnsw`|HNSWConfiguration|可选| |HNSWConfiguration|
|`fulltext_config`|FulltextIndexConfig|可选|全量搜索索引配置。其含有参数 `analyzer` 和 `properties`，都为 string 类型， 如果未提供，则使用默认值，默认为 `analyzer=’ik’`。有关其配置详细介绍，请参考 [CREATE INDEX](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000004479548) 的 parser_properties 内容。|`FulltextIndexConfig(analyzer='ik', properties={'ik_mode':'max_wor     ds'}))`|
...
config = Configuration(hnsw=HNSWConfiguration(dimension=384), fulltext_config=FulltextIndexConfig(analyzer='ik', properties={'ik_mode':'max_wor     ds'})) 
collection = client.create_collection(
    name="my_collection4",
    configuration=config,
...
* [检查 Collections 是否存在](360.has-collection-of-api.md)
