...
本示例演示相比 `query()`，`hybridSearch()` 的优势。

`hybridSearch()` 主要有以下优势：

* 支持同时执行全文搜索与向量相似性搜索

* 可分别为全文搜索和向量搜索配置过滤条件

* 通过 Reciprocal Rank Fusion 算法合并两种搜索的排序结果，提升相关性。
...
4. 使用`query()` 和 `hybridSearch()`进行查询，并对比结果。
5. 打印结果。
...
* 关于 seekdb-js 支持的 API 接口，参见 [API 参考](../50.apis-of-seekdb-js/10.api-overview-of-seekdb-js.md)。

* [Simple 示例](10.seekdb-js-simple-sample.md)

* [Complete 示例](50.seekdb-js-complete-sample.md)